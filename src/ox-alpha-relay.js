...
async function resolveSession(payload) {
  const profileId = 'ox-alpha-default';
...
function currentUserContent(requestText, attachments) {
  ...
  for (const attachment of attachments) {
    if (attachment.kind === 'image') {
      ...
      content.push({ type: 'file', file: { filename: attachment.name, file_data: attachment.dataUrl } });
...
function upstreamDetail(body, raw) {
  return clean(body?.error?.message || body?.error?.detail || body?.detail || body?.message || raw || '', 1400).trim() || null;
}

export function oxAlphaHealth() {
...
export async function oxAlphaRelay(req, res) {
  ...
  try {
    ...
    const status = message.startsWith('unsupported_attachment_type:') ? 415 : 413;
  ...
  try {
    ...
    const recorded = await recordConversationTurn({
      ...
      responseText: text,
      yukiState: { renderer: 'ox-alpha', provider: 'openrouter', isolated_from_yuki: true, attachment_count: attachments.length, model: DEFAULT_MODEL },
    });
