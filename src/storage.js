...
import { ensureNeonMemorySchema, neonMemoryConfigured, readResultTrace, writeResultMetadata, writeResultResponse, writeResultStart } from './neon-memory.js';
...
const ensureNeonReady=()=>{if(!neonSchemaReadyPromise)neonSchemaReadyPromise=ensureNeonMemorySchema();return neonSchemaReadyPromise;};
...
export async function readMeta(id){
  if(neonMemoryConfigured()){try{await ensureNeonReady();const row=await readResultTrace(id);if(row?.metadata)return row.metadata;}catch{}
  }
...
export async function readText(id,name){
  if(neonMemoryConfigured()){try{await ensureNeonReady();const row=await readResultTrace(id);const v=name==='request'?row?.request_text:name==='response'?row?.response_text:null;if(typeof v==='string')return v;}catch{}
  }
...
export async function finalizeMetadata(id,metadata){
  if(neonMemoryConfigured()){try{await ensureNeonReady();await writeResultMetadata(id,metadata);return true;}catch{await cacheSet(id,'metadata',metadata);return false;}}
