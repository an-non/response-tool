...
export async function ensureLegacySessionCompatibility(profileId, sessionId) {
  ...
  const shouldReconcile = !row || legacyShape || reportedTurnCount === 0 || missingRecent;
  ...
  if (!row && turnCount === 0) return { reconciled: false, turn_count: 0, reason: 'new_session' };
